...
beforeEach(function (): void {
    ...
    $this->otherShards = collect($this->shards)->reject(fn ($s): bool => $s === $this->shard);
});

it('stores a short URL in the correct shard', function (): void {
    $response = $this->postJson(route('api.short-urls.index'), $this->payload);
    ...
    $this->otherShards->each(
        fn ($shard): Pest\Expectation => expect(ShortUrl::on($shard)->count())->toBe(0)
    );
});

it('returns 409 if short URL already exists', function (): void {
